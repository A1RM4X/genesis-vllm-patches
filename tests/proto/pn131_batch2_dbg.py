...
for N in (20000,):
    ...
    r0 = correr(k2); rep_ok = all(all(bool((a == b).all()) for a, b in zip(r0, correr(k2))) for _ in range(3))
    ...
    dm_ = (r0[2].long() != em)
    if dm_.any():
        cs, rs = dm_.nonzero(as_tuple=True)
        print("  m distinto en", dm_.sum().item(), "celdas; filas", sorted(set(rs.tolist()))[:12], "paginas", sorted(set(cs.tolist()))[:6],
              "ej kernel", r0[2][cs[0], rs[0]].item(), "emul", em[cs[0], rs[0]].item(), flush=True)
    t1 = sorted(medir(lambda: k1.lanzar((NCH, 2), args, shared=P.SH_H)) for _ in range(3))[1]
    t2 = sorted(medir(lambda: k2.lanzar((NCH, 2), args, shared=P.SH_H)) for _ in range(3))[1]
    # salida completa y error vs float con el kernel nuevo
    P._kernels()["main"] = k2
    P._decode_uniforme(impl, qq, kv, md, out, 1, 4, False)
    Nq = N
    ref = A.referencia(q.repeat(reps, 1, 1)[:N], kk, vv, torch.arange(N - 4, N, device=dev)).float() if False else None
    print(f"N={N}: repeticiones iguales={rep_ok} | exacto vs emulador (hi,lo,m,S)={exact} | kernel viejo {t1:.3f} ms -> dos pasadas {t2:.3f} ms", flush=True)
