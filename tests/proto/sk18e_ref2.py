...
def streaming(zp, mq, svf, V8, T, modo, rescale=True, vsh=0):
    ...
    for k0 in range(0, N, T):
        ...
        wp = (w * svf[None, k0:k0 + T] + (16384 >> vsh)) >> (15 - vsh)
    ...
    for ROT in (0,):
        ...
        for p0 in torch.linspace(8000, Nt - 5, 8).long().tolist():
            ...
            for h in range(2):
                ...
                for RK, RV in ((1, 1), (4, 4), (16, 16), (64, 64)):
                    kref = sk.max() * RK; vref = sv.max() * RV
                    skf = torch.round(sk / kref * 32767).long(); svf = torch.round(sv / vref * 32767).long()
                    lr = {1: 0, 4: 2, 16: 4, 64: 6}[RK]
                    zp = ((z >> (8 - lr)) * skf[None]) >> 11
                    assert int(((z.abs() >> (8 - lr)) * skf.max()).max()) < 2**31
                    zp = torch.where(torch.arange(N, device=dev)[None] > lim[:, None], torch.full_like(zp, -(1 << 28)), zp)
                    mq = torch.round(kref * sq * 1023 / 16.0 * 65536 / (1 << lr)).clamp(1, (1 << 31) - 1).long()
                    o, S, nr = streaming(zp, mq, svf, V8, 256, "cuad2", vsh=lr)
                    out = o.double() * vref / (1 << lr) / S.double().clamp_min(1)[:, None]
                    e = ((out - r).norm(dim=-1) / r.norm(dim=-1)).mean().item()
                    acum.setdefault(f"rot={ROT} ref=max x{RK:<3d}", []).append(e)
    print(f"\ncapa {capa}")
    for kx, vx in acum.items():
        print(f"  {kx:28s} {100*sum(vx)/len(vx):.3f}%", flush=True)
