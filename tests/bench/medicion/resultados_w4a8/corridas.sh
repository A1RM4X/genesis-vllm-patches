...
log "inicio"
# 1) noon W4A16 (sin la variable)
sed -i 's#^      - VLLM_MARLIN_INPUT_DTYPE=int8  \# CORRIDA#      \# - VLLM_MARLIN_INPUT_DTYPE=int8  \# CORRIDA#' $NOON
if arrancar $NOON qwen38-27b-noon-w4a8; then log "noon_w4a16 arriba"; medir noon_w4a16 1; else log "noon_w4a16 NO ARRANCO"; docker logs --tail 80 $CN > $R/noon_w4a16_fallo.log 2>&1; fi
# 2) noon W4A8
sed -i 's#^      \# - VLLM_MARLIN_INPUT_DTYPE=int8  \# CORRIDA#      - VLLM_MARLIN_INPUT_DTYPE=int8  \# CORRIDA#' $NOON
if arrancar $NOON qwen38-27b-noon-w4a8; then log "noon_w4a8 arriba"; medir noon_w4a8 1; else log "noon_w4a8 NO ARRANCO"; docker logs --tail 80 $CN > $R/noon_w4a8_fallo.log 2>&1; fi
# 3) produccion Ar4ikov (restaura y re-mide velocidad en las mismas condiciones)
if arrancar $PROD qwen38-27b-ar4ikov-awq; then log "prod arriba"; medir prod_ar4ikov 0; else log "PROD NO ARRANCO"; docker logs --tail 80 $CN > $R/prod_fallo.log 2>&1; fi
cd /tmp/g115 && python3 sondeos.py --comparar noon_w4a16 noon_w4a8 > $R/sondeos_w4a8_vs_w4a16.txt 2>&1
python3 sondeos.py --comparar fp16 fp8 noon_w4a16 noon_w4a8 > $R/sondeos_vs_ar4ikov_fp16.txt 2>&1
log "fin"
