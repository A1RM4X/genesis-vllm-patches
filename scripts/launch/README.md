...
# 4. Health check
curl http://localhost:8000/health -H "Authorization: Bearer <REDACTADO>"
```
...
- **API key**: `<REDACTADO>` (CHANGE for production — see Security note below)
...
Default `--api-key <REDACTADO>` is for **local development only**. For any deployment beyond localhost:

1. Generate a strong key: `openssl rand -hex 32`
2. Edit the script — replace `--api-key <REDACTADO>` with `--api-key $YOUR_KEY`
