...
curl -s http://localhost:8000/v1/chat/completions \
  -H "Authorization: Bearer <REDACTADO>" \
...
> **API key note:** the launch scripts include `--api-key <REDACTADO>` so the `Authorization: Bearer <REDACTADO>` header is required. If you launched **without** `--api-key`, drop the header (and any `OPENAI_API_KEY` you set client-side). If you set a different key, replace it.
