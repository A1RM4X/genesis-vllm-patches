...
| API key | matches the `--api-key` your server was started with | Default: `<REDACTADO>` |
...
| `--api-key` | server API key | `<REDACTADO>` |
...
| Bench finishes in 30 seconds with empty per-prompt stats | All trials failed; check the JSON for `error` fields | Usually a model-name or auth-key mismatch. Verify `curl -H "Authorization: Bearer <REDACTADO>" http://host:port/v1/models` returns 200. |
