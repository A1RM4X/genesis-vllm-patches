...
# --- quant _sk08_conv1d_silu_kernel: 1 variante(s) de PTX embebido ---
...
_QVAR0_0 = _Nativo(
    "sk08_ssm_control/_sk08_conv1d_silu_kernel/blk-",
    _QPTX0_0, "_sk08_conv1d_silu_kernel",
    warps=8, shared=2048,
...
def _q0_impl(grid: list[int], x_ptr: torch.Tensor, w_ptr: torch.Tensor, bias_ptr: torch.Tensor, state_ptr: torch.Tensor, out_ptr: torch.Tensor, D: int, stride_x_b: int, stride_state_b: int, stride_state_d: int, stride_state_w: int, stride_out_b: int, WIDTH: int, BLOCK_D: int) -> None:
    ...
    args = (x_ptr, w_ptr, bias_ptr, state_ptr, out_ptr, D, stride_x_b, stride_state_b, stride_state_d, stride_state_w, stride_out_b, WIDTH, BLOCK_D)
    for _v in _POR_Q0:
        if all(args[_p] == _x for _p, _x in _v.horneado.items()):
            return _v(tuple(grid), *args)
    raise ValueError(
        "genesis_sk08_ssm_control_q0: no hay PTX embebido para estos constexpr; "
...
if not hasattr(torch.ops.vllm, "genesis_sk08_ssm_control_q0"):
...
# --- quant _sk08_gated_delta_rule_kernel: 1 variante(s) de PTX embebido ---
...
_QVAR1_0 = _Nativo(
    "sk08_ssm_control/_sk08_gated_delta_rule_kernel/blk-",
    _QPTX1_0, "_sk08_gated_delta_rule_kernel",
    warps=4, shared=512,
...
def _q1_impl(grid: list[int], qkv_ptr: torch.Tensor, a_ptr: torch.Tensor, b_ptr: torch.Tensor, A_log_ptr: torch.Tensor, dt_bias_ptr: torch.Tensor, state_ptr: torch.Tensor, o_ptr: torch.Tensor, stride_qkv_b: int, stride_a_b: int, stride_state_b: int, stride_o_b: int, SCALE: float, H: int, HV: int, K: int, V: int, SOFTPLUS_THRESHOLD: float) -> None:
    """Cuerpo del custom op: elige la variante y lanza el PTX."""
    args = (qkv_ptr, a_ptr, b_ptr, A_log_ptr, dt_bias_ptr, state_ptr, o_ptr, stride_qkv_b, stride_a_b, stride_state_b, stride_o_b, SCALE, H, HV, K, V, SOFTPLUS_THRESHOLD)
    for _v in _POR_Q1:
        if all(args[_p] == _x for _p, _x in _v.horneado.items()):
            return _v(tuple(grid), *args)
    raise ValueError(
        "genesis_sk08_ssm_control_q1: no hay PTX embebido para estos constexpr; "
...
# choque cuando el modulo se importa dos veces con nombres distintos,
# como hace el gate de tools/monolitizar.py.
if not hasattr(torch.ops.vllm, "genesis_sk08_ssm_control_q1"):
    from vllm.utils.torch_utils import direct_register_custom_op
    direct_register_custom_op(
