...
//
// SK-17 — rotacion Hadamard de activaciones en INT8 exacto (mma s8 m16n8k32).
//
// H_256 = S / 16 con S en {-1, +1}: la matriz entra en int8 SIN error. Con x
// cuantizada int8 por token (xq, sx):
//
//     y_int[m', i] = sum_k xq[m', k] * S[i, k]        (int32, |y| <= 256*127)
//     y = y_int * sx / 16
//
// A = xq vista como [M*NG, 256] (bloques de 256 de cada token), B = S [256, 256].
// Salida int16 [M*NG, 256]: entra holgado (32.512 < 32.767). La cuantizacion a
// int4 por grupo que sigue no necesita sx (round(y*7/max|y|) es invariante a
// escala), asi que todo el camino es entero hasta el redondeo final.
//
// Esqueleto de SK-12 sin la segunda matriz, sin escalas y sin SiLU.
#include <cuda_fp16.h>
...
#pragma unroll
        for (int h = 0; h < 2; ++h) {
            ...
            if (gm >= M) continue;
            short* orow = out + (size_t)gm * N;
#pragma unroll
            for (int j = 0; j < NTILES; ++j) {
                const int c0  = bn + wn + j * 8 + tig * 2;
                const int idx = h * 2;
                if (c0 < N)     orow[c0]     = (short)accG[i][j][idx];
                if (c0 + 1 < N) orow[c0 + 1] = (short)accG[i][j][idx + 1];
            }
        }
}
