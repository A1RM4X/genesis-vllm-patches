...
//
// SK-15b — SK-15 (gate_up+SiLU W4A4, escalas por grupo de 256) con el VOLCADO
// DESFASADO un tile.
//
// Medido en SK-15: cada volcado del acumulador (sacar los int32 del pipe
// tensorial al ALU) cuesta ~1,3 ms en la forma real y escala lineal con la
// cantidad de volcados (cada 1/2/4/10/20 tiles: 33,4/19,1/14,0/10,9/8,3 ms).
// Aca hay DOS juegos de acumuladores que se alternan por paridad de tile: el
// tile t acumula en el juego t%2 y al terminar se vuelca el juego del tile t-1,
// cuyos mma ya estan en vuelo desde antes. El ALU no espera al pipe tensorial
// del tile que acaba de emitir.
//
// VOLCADO=0 flotante con escalas reales (el kernel), VOLCADO=1 entero con shift
// constante (diagnostico de costo, resultado sin sentido).
#define S4 1
#define SK14_MMA_OP "mma.sync.aligned.m16n8k64.row.col.satfinite.s32.s4.s4.s32 "
#ifndef VOLCADO
#define VOLCADO 0
#endif
#ifndef DESFASE
#define DESFASE 0
#endif
// ESC_SHARED=1: las escalas del grupo de cada tile viajan a shared con cp.async
// junto con A y B (medido en SK-15: leerlas de memoria global en el volcado
// costaba ~50 de los ~80 ms).
#ifndef ESC_SHARED
#define ESC_SHARED 1
#endif
// (Documentacion heredada de SK-12 abajo.)
// SPDX-License-Identifier: Apache-2.0
//
...
static_assert(STAGES * (BM * BK + BN * 2 * BK) + STAGES * 4 * (BM + 2 * BN) <= 101376,
...
{
    const int NG = K / BK;
    // Filas de las escalas traspuestas, rellenadas a multiplo de 4 (ESC_SHARED).
    const int Mp = (M + 3) & ~3, Np = (N + 3) & ~3;
    ...
    // Escalas por etapa: [BM] de A, [BN] de gate, [BN] de up (fp32).
    float (*sS)[BM + 2 * BN] =
        (float (*)[BM + 2 * BN]) (_smem + STAGES * BM * BK + STAGES * BN * 2 * BK);
    ...
    int accG[2][MTILES][NTILES][4];
    int accU[2][MTILES][NTILES][4];
    float faccG[MTILES][NTILES][4];
    float faccU[MTILES][NTILES][4];
#if VOLCADO == 1
    int iaccG[MTILES][NTILES][4];
    int iaccU[MTILES][NTILES][4];
#endif
#pragma unroll
    for (int p = 0; p < 2; ++p)
#pragma unroll
...
#pragma unroll
            for (int e = 0; e < 4; ++e) {
                accG[p][i][j][e] = 0; accU[p][i][j][e] = 0;
                faccG[i][j][e] = 0.0f; faccU[i][j][e] = 0.0f;
#if VOLCADO == 1
                iaccG[i][j][e] = 0; iaccU[i][j][e] = 0;
#endif
            }
...
#if ESC_SHARED
#define SK15_CARGA_ESC(BUF, K0)                                                \
        {                                                                      \
            /* sa, sg, su llegan TRASPUESTAS: [NG, M] / [NG, N], asi el grupo  \
               g es contiguo a lo largo de las filas y viaja de a 4 floats. */ \
            const int g_ = (K0) / BK;                                          \
            for (int v = tid; v < BM / 4; v += nthreads) {                     \
                const int r = v * 4, gm = bm + r;                              \
                /* escalas rellenadas a multiplo de 4 filas: copia siempre 16 B */ \
                SK12_LOAD_A(sdir(&sS[BUF][r]),                                 \
                    (const signed char*)(sa + (size_t)g_ * Mp + (gm < M ? gm : 0)), gm < M ? 16 : 0); \
            }                                                                  \
            for (int v = tid; v < BN / 4; v += nthreads) {                     \
                const int r = v * 4, gn = bn + r;                              \
                SK12_LOAD_B(sdir(&sS[BUF][BM + r]),                            \
                    (const signed char*)(sg + (size_t)g_ * Np + (gn < N ? gn : 0)), gn < N ? 16 : 0); \
                SK12_LOAD_B(sdir(&sS[BUF][BM + BN + r]),                       \
                    (const signed char*)(su + (size_t)g_ * Np + (gn < N ? gn : 0)), gn < N ? 16 : 0); \
            }                                                                  \
        }
#else
#define SK15_CARGA_ESC(BUF, K0)
#endif
#if ESC_SHARED
#define SK15_AS(GM_, G_) sS[buf][(GM_) - bm]
#define SK15_WG(GN_, G_) sS[buf][BM + (GN_) - bn]
#define SK15_WU(GN_, G_) sS[buf][BM + BN + (GN_) - bn]
#else
#define SK15_AS(GM_, G_) ((GM_) < M ? sa[(size_t)(GM_) * NG + (G_)] : 0.0f)
#define SK15_WG(GN_, G_) ((GN_) < N ? sg[(size_t)(GN_) * NG + (G_)] : 0.0f)
#define SK15_WU(GN_, G_) ((GN_) < N ? su[(size_t)(GN_) * NG + (G_)] : 0.0f)
#endif
        ...
        {                                                                      \
            ...
            for (int i = 0; i < MTILES; ++i) {                                 \
                ...
                for (int j = 0; j < NTILES; ++j) {                             \
                    ...
                    SK12_MMA1(accG[par][i][j], A[i], BG[j])                    \
                    if ((CARGAR) && t_ < MTILES) SK12_LDA(KSIG, t_, NA)        \
                    SK12_MMA1(accU[par][i][j], A[i], BU[j])                    \
...
// Vuelca el juego SET_ (grupo G_) al acumulador float y lo pone a cero.
#define SK15_VOLCAR(SET_, G_)                                                  \
    {                                                                          \
        const int g_ = (G_), set_ = (SET_);                                    \
        const int gid_ = lane >> 2, tig_ = lane & 3;                           \
        _Pragma("unroll")                                                      \
        for (int i = 0; i < MTILES; ++i)                                       \
        _Pragma("unroll")                                                      \
        for (int h = 0; h < 2; ++h) {                                          \
            const int gm = bm + wm + i * 16 + gid_ + h * 8;                    \
            const float as = SK15_AS(gm, g_);                                  \
            _Pragma("unroll")                                                  \
            for (int j = 0; j < NTILES; ++j)                                   \
            _Pragma("unroll")                                                  \
            for (int c = 0; c < 2; ++c) {                                      \
                const int gn = bn + wn + j * 8 + tig_ * 2 + c;                 \
                const int e = h * 2 + c;                                       \
                SK15_UNO(set_, i, j, e, as, gn, g_)                            \
                accG[set_][i][j][e] = 0;                                       \
                accU[set_][i][j][e] = 0;                                       \
            }                                                                  \
        }                                                                      \
...
#if VOLCADO == 1
#define SK15_UNO(S_, I_, J_, E_, AS_, GN_, G2_)                                \
    (void)(AS_);                                                               \
    iaccG[I_][J_][E_] += accG[S_][I_][J_][E_] << 3;                            \
    iaccU[I_][J_][E_] += accU[S_][I_][J_][E_] << 3;
#else
#define SK15_UNO(S_, I_, J_, E_, AS_, GN_, G2_)                                \
    {                                                                          \
        const float wg_ = SK15_WG(GN_, G2_);                                   \
        const float wu_ = SK15_WU(GN_, G2_);                                   \
        faccG[I_][J_][E_] += (float)accG[S_][I_][J_][E_] * ((AS_) * wg_);      \
        faccU[I_][J_][E_] += (float)accU[S_][I_][J_][E_] * ((AS_) * wu_);      \
    }
#endif
...
#pragma unroll
    ...
    for (int k00 = 0; k00 < K; k00 += 2 * BK)
#pragma unroll
    for (int par = 0; par < 2; ++par) {
        const int k0 = k00 + par * BK;
...
#if DESFASE
        // Juego del tile anterior: sus mma ya estaban en vuelo.
        if (k0 > 0) SK15_VOLCAR(1 - par, k0 / BK - 1)
#else
        SK15_VOLCAR(par, k0 / BK)
#endif
        __syncthreads();
        buf = (buf + 1) % STAGES;
    }
#if DESFASE
    // Ultimo tile (NG par: juego 1).
    SK15_VOLCAR(1, NG - 1)
#endif
#undef SK12_CARGA
...
#pragma unroll
            for (int j = 0; j < NTILES; ++j) {
                ...
                const int idx = h * 2;
#if VOLCADO == 1
                const __half2 g = __floats2half2_rn((float)iaccG[i][j][idx] * 1e-4f, (float)iaccG[i][j][idx + 1] * 1e-4f);
                const __half2 u = __floats2half2_rn((float)iaccU[i][j][idx] * 1e-4f, (float)iaccU[i][j][idx + 1] * 1e-4f);
#else
                const __half2 g = __floats2half2_rn(faccG[i][j][idx], faccG[i][j][idx + 1]);
                const __half2 u = __floats2half2_rn(faccU[i][j][idx], faccU[i][j][idx + 1]);
#endif
