...
//
// SK-18h/union — (v3: mg precalculado y tramos vacios salteados; apto para CUDA graph con NCH fijo)
...
sk18h_union(
    ...
    const int* __restrict__ mgv,      // [R] maximo global por fila (max_c om)
    ...
    int R, int NCH, int CPG)
{
    ...
    long long sacc = 0;
    for (int c = c0; c < c1; ++c) {
        const int mc_ = om[(size_t)c * R + r];
        if (mc_ <= -(1 << 29)) continue;                     // tramo sin keys visibles
        int dm = mg - mc_;
        dm = dm < dc ? dm : dc;
