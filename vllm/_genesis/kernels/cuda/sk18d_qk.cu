...
//
// SK-18d/1 — Q.K int8 x int8 del decode con la escala de key y la mascara causal
// plegadas en el epilogo (sin punto flotante, todo 32 bits):
//
//     out[m, n] = ((sum_k A[m,k]*B[n,k]) >> 8) * skf[n] >> 11     si n <= lim[m]
//               = PAD = -(1 << 28)                                  si no
//
// skf int16 [N] = escala de key relativa al maximo (Q15); lim int32 [M] = ultima key
// visible por query (causal + MTP); out tiene filas de KS >= N (relleno PAD afuera). |acc| <= 256*127*127 -> (acc>>8)*32767 < 2^30.
// Tiles BM=32 (M de decode ~24; BM=256 era 0,243 -> 0,195 ms en la cadena @57k).
// Asi la salida entra directo a SK-18d/2 sin pegamento de torch en el medio.
// S4=1: mma.sync.aligned.m16n8k64 s4 (A y B en nibbles, K en BYTES = elementos/2).
// Mismo layout de fragmentos que s8 (medido con sondas, kernels/mma_layout.py).
#ifndef S4
#define S4 0
#endif
#if S4
#define SK18_MMA_OP "mma.sync.aligned.m16n8k64.row.col.satfinite.s32.s4.s4.s32 "
#else
#define SK18_MMA_OP "mma.sync.aligned.m16n8k32.row.col.satfinite.s32.s8.s8.s32 "
#endif
#include <cuda_fp16.h>
...
sk18d_qk(
    ...
    const short* __restrict__ skf,
    const int* __restrict__ lim,
    int* __restrict__ out,
    int M, int N, int K, int KS, int NH)
{
    ...
    // NH cabezas KV en un lanzamiento: la grilla y son NH x TM tiles de query; M y N
    // son POR CABEZA y A [NH*M,K], B [NH*N,K], skf [NH*N], lim/out [NH*M] van apilados.
    const int TM = gridDim.y / NH;
    const int hh = pid_m / TM;
    const int bm = (pid_m % TM) * BM;
    const signed char* Ah = A + (size_t)hh * M * K;
    const signed char* Wgh = Wg + (size_t)hh * N * K;
...
#pragma unroll
            for (int e = 0; e < 4; ++e) { accG[i][j][e] = 0; }
    ...
    do {                                                                       \
        ...
        for (int v = tid; v < BN * vpr; v += nthreads) {                       \
            ...
            SK12_LOAD_B(sdir(&sB[BUF][r][swzB(r, 0, c)]),                      \
                Wgh + (size_t)cn * K + (K0) + c, gn < N ? 16 : 0);              \
...
// FLUSH (diagnostico SK-18e): operacion de registro sobre los acumuladores del mma
// entre pasos (lo que haria un maximo corriente / reescalado). 1 = por tile de BK, 2 = por paso.
#ifndef FLUSH
#define FLUSH 0
#endif
    const int sc_ = (int)skf[0] + 1;   // 32768 en el test: reescalado identidad no plegable
#define SK18_TOCAR_ACC                                                         \
        _Pragma("unroll") for (int i_ = 0; i_ < MTILES; ++i_)                 \
        _Pragma("unroll") for (int j_ = 0; j_ < NTILES; ++j_)                 \
        _Pragma("unroll") for (int e_ = 0; e_ < 4; ++e_)                      \
            accG[i_][j_][e_] = ((accG[i_][j_][e_] >> 15) * sc_)                \
                             + (((accG[i_][j_][e_] & 0x7fff) * sc_) >> 15);
...
#pragma unroll
        for (int h = 0; h < 2; ++h) {
            ...
            if (gm >= M) continue;
            int* orow = out + ((size_t)hh * M + (size_t)gm) * KS;   // KS = paso de fila de out (K de SK-18d/2)
#pragma unroll
            for (int j = 0; j < NTILES; ++j) {
                const int c0  = bn + wn + j * 8 + tig * 2;
                const int idx = h * 2;
                const int lm = lim[hh * M + gm];
                if (c0 < N)     orow[c0]     = c0 <= lm ? ((accG[i][j][idx] >> 8) * (int)skf[hh * N + c0]) >> 11 : -(1 << 28);
                if (c0 + 1 < N) orow[c0 + 1] = c0 + 1 <= lm ? ((accG[i][j][idx + 1] >> 8) * (int)skf[hh * N + c0 + 1]) >> 11 : -(1 << 28);
            }
        }
}
