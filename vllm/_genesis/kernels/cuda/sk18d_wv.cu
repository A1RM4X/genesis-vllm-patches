...
//
// SK-18d/2 — softmax ENTERO + sum(w*v) del decode, version rapida de SK-18b.
// Perfil de SK-18b (M=24, 57k keys, una cabeza): 0,190 ms; sin mma 0,177; con
// peso trivial 0,097 -> el mma es ~7%, la mitad es el calculo escalar del peso.
// Cambios (resultado identico bit a bit):
//   * todo en 32 bits: d = zmax - z' con corte temprano d >= dcap[m] -> peso 0
//     (dcap = primer d con indice 1023, calculado afuera), asi d*mq < 2^31 y la
//     mayoria de las keys (lejos del maximo) no tocan tabla ni multiplicacion;
//     w*svf son 15x15 bits.
//   * peso SIN RAMA (SIN_SALTO=1): la rama d < dcap se predice mal (46% de keys
//     con peso en capa 35): 0,100 -> 0,077 ms.
//   * tiles de query BM=32 WM=16 x 8 warps, tramos de CH=512: en decode M ~ 24 y
//     BM=64 dejaba 62% de filas de relleno en cada carga.
// Salto de bloques: tabla int32 [NCH] = tramos a calcular (primero los elegidos),
// nsel int32 [1] = cuantos; sumw int64 [NCH*tiles*hilos] = sum lut por hilo (fila tid/vpr). Denso: tabla = 0..NCH-1, nsel = NCH.
// Entradas: Sz = salida de SK-18d/1 (z' con PAD), zmax/mq/dcap int32 [M].
#include <cuda_fp16.h>
#include <cuda_pipeline.h>
// DIAG (perfil): 0 normal; 1 carga de peso trivial (sin tabla ni mult); 2 sin mma.
// SIN_SALTO=1: peso sin rama (d recortado a dcap, indice a 1023; lut[1023] = 0).
#ifndef SIN_SALTO
#define SIN_SALTO 1
#endif
#ifndef DIAG
...
static_assert(BM * BK / 16 == NWARPS * 32,
              "sumw es por hilo: cada hilo tiene que cargar exactamente una fila de 16 keys");
...
sk18d_wv(
    ...
    const int* __restrict__ tabla,
    const int* __restrict__ nsel,
    long long* __restrict__ sumw,
    int M, int N, int K, int CH, int NCH, int NH)
{
    ...
    // NH cabezas KV en un lanzamiento: blockIdx.y = ((hh*TM + tile) * NCH + tramo).
    // M por cabeza; zmax/mq/dcap [NH*M], Sz [NH*M,K], svf [NH*K], Vt [NH*N,K], out [NCH,NH*M,N].
    const int TM = gridDim.y / (NCH * NH);
    const int pid_mt = blockIdx.y / NCH;
    const int hh = pid_mt / TM;
    const int pid_m = pid_mt % TM;
    const int hm = hh * M;
    const int tramo = blockIdx.y % NCH;
    // Salto de bloques: la grilla cubre NCH tramos pero solo los nsel[0] primeros
    // de `tabla` se calculan (lista en GPU, sin sincronizar con el host); el resto
    // sale sin tocar nada (out tiene que venir en cero).
    if (tramo >= nsel[0]) return;
    const int kini = tabla[tramo] * CH;
...
#define SK12_CARGA(BUF, K0)                                                    \
    do {                                                                       \
        for (int v = tid; v < BM * vpr; v += nthreads) {                       \
            ...
            if (gm < M) {                                                      \
                const int zm_ = zmax[hm + gm];                                      \
                const int mq_ = mq[hm + gm];                                        \
                const int dc_ = dcap[hm + gm];                                      \
                const int* srow_ = Sz + ((size_t)hm + gm) * K;                        \
                for (int e_ = 0; e_ < 16; ++e_) {                              \
                    const int kk_ = (K0) + c + e_;                             \
                    int w_ = 0;                                                \
                    if (kk_ < kfin) {                                          \
                        const int d0_ = zm_ - srow_[kk_];                      \
                        if (SIN_SALTO) {                                       \
                            const int d_ = d0_ < dc_ ? d0_ : dc_;              \
                            int ix_ = (d_ * mq_ + 32768) >> 16;                \
                            ix_ = ix_ < 1023 ? ix_ : 1023;                     \
                            const int lw_ = lut[ix_];                          \
                            sumw_ += lw_;                                      \
                            w_ = (lw_ * (int)svf[hh * K + kk_] + 16384) >> 15;          \
                        } else if (d0_ < dc_) {                                \
                            const int ix_ = (d0_ * mq_ + 32768) >> 16;         \
                            w_ = (lut[ix_] * (int)svf[hh * K + kk_] + 16384) >> 15;     \
                        }                                                      \
                    }                                                          \
        ...
        for (int v = tid; v < BN * vpr; v += nthreads) {                       \
            ...
            SK12_LOAD_B(sdir(&sB[BUF][r][swzB(r, 0, c)]),                      \
                Vt + ((size_t)hh * N + cn) * K + (K0) + c, gn < N ? 16 : 0);              \
...
#define SK12_PASO(KSIG, A, AL, BG, BU, NA, NAL, NBG, NBU, CARGAR)              \
    ...
    // Suma de pesos de la tabla (sin escala de V) para normalizar afuera. Con
    // BM*BK/16 == hilos cada hilo carga exactamente UNA fila de 16 keys por tile
    // de BK (fila tid/vpr), asi que la suma es por hilo y no hay carreras.
    long long sumw_ = 0;
...
#undef SK12_CARGA
    ...
    if (bm + tid / vpr < M)
        sumw[((size_t)tramo * (size_t)(NH * TM) + (size_t)pid_mt) * (size_t)nthreads + (size_t)tid] = sumw_;
...
#pragma unroll
        for (int h = 0; h < 2; ++h) {
            ...
            if (gm >= M) continue;
            int* orow = out + (size_t)tramo * NH * M * N + ((size_t)hm + gm) * N;
            int* orow_lo = out_lo + (size_t)tramo * NH * M * N + ((size_t)hm + gm) * N;
#pragma unroll
            for (int j = 0; j < NTILES; ++j) {
