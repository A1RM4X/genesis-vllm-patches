...
//
// SK-18a — GEMM int8 x int8 -> int32 (paso Q.K del decode de SK-18, v1 en 3 lanzamientos).
//
//     out[m, n] = sum_k A[m, k] * B[n, k]        (int32, sin punto flotante)
//
// A = queries int8 rotadas (Hadamard, PN126) [M, 256]; B = keys int8 [N=L, 256].
// En decode M es chico (B x 4 posiciones x 6 cabezas Q por cabeza KV) y N es el
// contexto, asi que la grilla es de tiles de keys. Esqueleto de SK-17 (que es
// SK-12 sin la segunda matriz ni SiLU) con salida int32.
...
#pragma unroll
        for (int h = 0; h < 2; ++h) {
            ...
            if (gm >= M) continue;
            int* orow = out + (size_t)gm * N;
#pragma unroll
            for (int j = 0; j < NTILES; ++j) {
                const int c0  = bn + wn + j * 8 + tig * 2;
                const int idx = h * 2;
                if (c0 < N)     orow[c0]     = accG[i][j][idx];
                if (c0 + 1 < N) orow[c0 + 1] = accG[i][j][idx + 1];
            }
        }
}
