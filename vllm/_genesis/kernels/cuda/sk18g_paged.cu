...
//
// SK-18g — SK-18f sobre KV PAGINADA real (bloques fisicos de vLLM en cualquier orden).
//
// En este hibrido vLLM fija el bloque de atencion en 832 tokens (pagina >= pagina de
// mamba). Cada bloque de la grilla es UNA pagina logica: lee su id fisico de la tabla
// de bloques una vez y calcula la base; dentro de la pagina todo es contiguo.
// FlashInfer calcula una direccion por token (divmod por page_size + kv_indices); aca
// hay una sola indireccion cada 832 tokens.
//
// Layout del bloque fisico (lo escribe nuestro backend, no vLLM):
//   [0, KOFF)        K int8   [BS][NH][256]   por token (como vLLM NHD)
//   [KOFF, EOFF)     V int8   [NH][256][BS]   por DIMENSION: el cp.async de 16 B de la
//                                              fila de V^T sale directo, sin transponer
//                                              (FlashInfer transpone con ldmatrix.trans +
//                                              3 mariposas + byte_perm)
//   [EOFF, BLK)      escalas int16 [BS][NH][2] (skf, svf) por token
// Mismo tamano que fp8 [2][BS][NH][256] + 4*NH bytes por token.
#include <cuda_fp16.h>
...
sk18g_paged(
    ...
    const signed char* __restrict__ pool,   // [P, BLK] bloques fisicos
    const int* __restrict__ bt,             // [NCH] tabla de bloques de la secuencia
    ...
    int MB, int N, int K, int CHK, int NCH, int NH)   // CHK = BS (tokens por bloque)
{
    ...
    // Pagina fisica de este tramo: UNA carga global por bloque.
    const size_t BLK = (size_t)CHK * NH * QD * 2 + (size_t)CHK * NH * 4;
    const size_t KOFF = (size_t)CHK * NH * QD;
    const size_t EOFF = 2 * KOFF;
    const signed char* base = pool + (size_t)bt[tramo] * BLK;
...
#define SK18G_CARGA(ET, K0)                                                    \
    ...
    if (kini < kfin) { SK18G_CARGA(0, kini); } else { __pipeline_commit(); }
    int buf = 0;
    for (int k0 = kini; k0 < kfin; k0 += BK) {
        const int knext = k0 + BK;
        const int slot = (buf + 1) % 2;
        if (knext < kfin) { SK18G_CARGA(slot, knext); } else { __pipeline_commit(); }
