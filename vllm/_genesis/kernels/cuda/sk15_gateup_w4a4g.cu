...
//
// SK-15 — gate_up + SiLU en W4A4 con ESCALAS POR GRUPO de 256 (s4 m16n8k64).
//
// Parte de SK-14 (SK-12 con la instruccion s4). La diferencia es la escala:
// SK-14 desescala UNA vez al final con escala por token y por fila, y eso en
// W4A4 pierde demasiado (simulado en gate_up con Hadamard 256: KL 0,029 y logp
// -1,403 contra -1,280 con grupos de 256). Aca cada tile de k (BK=128 bytes =
// 256 nibbles) es exactamente un grupo: al terminar sus 4 pasos de mma el
// acumulador int32 se vuelca a un acumulador float multiplicado por
// sa[m,g]*sw[n,g] y se pone a cero.
//
//     out[m,n] = silu(sum_g (A_g . Wg_g) sa[m,g] sg[n,g])
//              *      sum_g (A_g . Wu_g) sa[m,g] su[n,g]
//
// Registros: SK-12 usaba 128 acumuladores int + 64 de fragmentos (192 de ~212).
// El acumulador float suma otros 128 y no entra, asi que los warps son de
// WM=32 x WN=16 (32 int + 32 float + 32 de fragmentos) y la grilla de warps es
// (BM/WM) x (BN/WN) = 8 x 4 = 32.
//
// Escalas: sa [M, NG], sg/su [N, NG] fp32, NG = K / BK. v1 las lee de memoria
// global en el volcado de cada tile.
#define S4 1
#ifndef SK15_DIAG
#define SK15_DIAG 0
#endif
// Diagnostico: volcar cada FLUSH_CADA tiles (1 = por grupo de 256).
#ifndef FLUSH_CADA
#define FLUSH_CADA 1
#endif
...
sk15_gateup_w4a4g(
    ...
    int M, int N, int K)
{
    const int NG = K / BK;   // grupos de 256 elementos (K en bytes)
...
#error "BK tiene que ser 64 o 128: el bucle interno esta desenrollado a mano"
        ...
        if (((k0 / BK) + 1) % FLUSH_CADA == 0 || k0 + BK >= K)
        // Volcado del grupo g = k0/BK: int32 -> float con sa[m,g]*s{g,u}[n,g].
        // La fila y la columna de cada acumulador son las mismas que usa el
        // epilogo de SK-12 (validado): fila gid + h*8, columna tig*2 + c.
        {
            const int g_ = k0 / BK;
...
#pragma unroll
                for (int h = 0; h < 2; ++h) {
                    ...
                    const float as = 1.0f;
#else
                    const float as = gm < M ? sa[(size_t)gm * NG + g_] : 0.0f;
#endif
#pragma unroll
                    for (int j = 0; j < NTILES; ++j)
#pragma unroll
                        for (int c = 0; c < 2; ++c) {
                            ...
                            const int e = h * 2 + c;
#if SK15_DIAG == 1
                            const float wg_ = 1.0f, wu_ = 1.0f;
#else
                            const float wg_ = gn < N ? sg[(size_t)gn * NG + g_] : 0.0f;
                            const float wu_ = gn < N ? su[(size_t)gn * NG + g_] : 0.0f;
#endif
#if SK15_DIAG == 2
                            // sin volcado float: solo acumular int (como SK-14)
                            (void)wg_; (void)wu_;
#elif SK15_DIAG == 4
                            // solo reset, SIN leer los acumuladores (resultado basura)
                            (void)wg_; (void)wu_;
                            accG[i][j][e] = 0;
                            accU[i][j][e] = 0;
#elif SK15_DIAG == 3 || SK15_DIAG == 5
                            // volcado ENTERO con shift constante (costo de la variante pow2)
                            (void)wg_; (void)wu_;
                            iaccG[i][j][e] += accG[i][j][e] << 3;
                            iaccU[i][j][e] += accU[i][j][e] << 3;
                            accG[i][j][e] = 0;
                            accU[i][j][e] = 0;
#else
                            faccG[i][j][e] += (float)accG[i][j][e] * (as * wg_);
                            faccU[i][j][e] += (float)accU[i][j][e] * (as * wu_);
                            accG[i][j][e] = 0;
                            accU[i][j][e] = 0;
#endif
                        }
...
#pragma unroll
            for (int j = 0; j < NTILES; ++j) {
                ...
                const int idx = h * 2;
                // Las escalas ya se aplicaron grupo a grupo: el acumulador float
                // ES la preactivacion.
#if SK15_DIAG == 3 || SK15_DIAG == 5
