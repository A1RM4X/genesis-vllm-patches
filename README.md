...
# 4. Health check
curl http://localhost:8000/v1/models -H "Authorization: Bearer <REDACTADO>"
...
The default targets `localhost:8000` with API key `<REDACTADO>`. Override with `--host`, `--port`, `--api-key`, `--model`. See `--help` for the full flag list.
